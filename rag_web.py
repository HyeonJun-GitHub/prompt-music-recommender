...
.stApp {{
  ...
  background-size: contain,cover;  /* 첫 번째 이미지는 cover, 두 번째 이미지는 contain */
  background-position: left 100px right 100px top 500px,center;  /* 첫 번째 이미지는 중앙, 두 번째 이미지는 우측 상단 */
  background-repeat: no-repeat, no-repeat;  /* 둘 다 반복 없음 */
  background-origin: padding-box;  /* 배경 이미지를 패딩 영역부터 시작 */
  padding: 200px,0px;  /* 패딩을 설정하여 배경 이미지가 이 안에서 표시되도록 */
}}
</style>
'''
